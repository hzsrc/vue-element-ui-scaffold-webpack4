...
const webpackConfig = merge(baseWebpackConfig, {
    ...
    output: {
        ...
        filename: utils.assetsPath('h5/js/[name].[contenthash:8].js'),
        chunkFilename: utils.assetsPath('h5/js/[id].[contenthash:8].js'),
        clean: true
    },
    plugins: [
        ...
        new MiniCssExtractPlugin({
            ...
            ignoreOrder: true,
            filename: 'h5/css/[name].[contenthash:8].css',
            chunkFilename: 'h5/css/[id].[contenthash:8].css'
        }),
        new CssMinimizerWebpackPlugin({
            minimizerOptions: {
                sourceMap: config.build.productionSourceMap ? getSrcMapOpt() : false
            }
        }),
...
if (config.build.productionSourceMap) {
    ...
    */
    webpackConfig.plugins.push(
        new webpack.SourceMapDevToolPlugin(getSrcMapOpt())
    )
}
...
function getSrcMapOpt() {
    return {
        filename: `${config.build.sourceMapPath}/[contenthash].txt`,
        append: '\n//# sourceMappingURL=../api/cm/m?n=[contenthash]', // undefined会自动加载源码映射，生产环境慎用。false时不会
    }
}
