    ...
    <div class="zrest">
        ...
        <el-form labelWidth="120px">
            <el-form-item label="接口选择">
                ...
                >
                    <el-option
                        v-for="(item, index) in config.apiList"
                        :key="index"
                        :label="item.text"
                        :value="item.url"
                    >
            ...
            <el-form-item label="">
                <el-button @click="doRequest" type="primary"
                >发起请求
                </el-button
                >
            </el-form-item>
        ...
        <el-form labelWidth="120px">
            <el-form-item label="结果">
                <div class="pre">
                    {{ response.result }}<br/>{{ response.status }}
                </div>
            </el-form-item>
...
<script>
    ...
    import { ElRow, ElInput, ElButton, ElForm, ElFormItem, ElSelect, ElOption } from 'element-plus'
    ...
    export default {
        ...
        created() {
            ...
            const resp = (res) => {
                ...
                const headers = Object.assign(
                    {},
                    config.headers[this.request.method.toLowerCase()]
                );
                ...
                this.response.result =
                    this.request.method + ' ' + res.request.responseURL;
                this.response.status =
                    res.status ||
                    (res.response && res.response.status) + ' ' + res.message;
        ...
        methods: {
            ...
            setApiData() {
                const index = this.config.apiList.findIndex(
                    (api) => api.url == this.request.url
                );
                if (index > -1) {
                    this.request.data = JSON.stringify(
                        this.config.apiList[index].data,
                        null,
                        4
                    );
                }
            ...
            doRequest() {
                try {
                    ...
                    httpUtil[method](this.request.url, data, {
                        maskOptions: false,
                    });
                } catch (e) {
                                ...
                                function parseToData(list) {
                                    ...
                                    list.forEach((p) => {
                                        if (p.dataType == 'object')
                                            r[p.identifier] = parseToData(
                                                p.parameterList
                                            );
                                        else r[p.identifier] = types[p.dataType];
                                    });
                                    return r;
                                }
                            });
